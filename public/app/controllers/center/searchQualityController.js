app.controller('searchQualityController', function ($scope, $filter, $rootScope, $location, procedureService, filterService) {
    ...
    function generateWhereQuery() {
        var query = ' where length(mobile)>1 and status not in("\'"ANULADA"\'") and statusTicket in("\'"Derivado"\'","\'"Notificado"\'") ',
            orderQuery = '';
